...
import { SigninService, UsernameVerification, EmailVerification } from '../services/auth.services'
    ...
    SigninService(identity, password)
        .then(
            ...
            errorMessage => {
              dispatch(hideLoading())
              dispatch(toggleBusy(false))
              dispatch(showSigninFormError('Oops!', errorMessage))
            })
  }
...
export const showSigninFormError = (title, message) => {
...
export const showSignupFormError = (title, message) => {
  return { type: SHOW_SIGNUP_ERROR, error: { title: title, content: message } }
}
